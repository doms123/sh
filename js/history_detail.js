$(document).ready(function() {
	...
	pageTitleAndLoader('History Details');
	...
	function viewOfferConversation() {
		$.ajax({
			...
			success : function(data) {
				...
				$(".paymentHidden").val(offerAmount);
				$(".receiverHidden").val(data.offerNegotiatorEmail);
				...
				if(data.offerAddedByID == loginUserID) { // you are the offer Creator
					...
					html += '<p>';
						...
						if(data.offererAccepted == 0) {
							html += '<span class="checkbox"><i class="ion-checkmark-circled green"></i></span>';
						}else {
