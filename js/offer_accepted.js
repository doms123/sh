$(document).ready(function() {
	...
	pageTitleAndLoader('Accepted Offer');
	...
	function loadTodo() {
		$.ajax({
			...
			success: function(data) {
				...
				if(data.offerAddedByID == loginUserID) { // you are the offer Creator
					...
					html += '<p>';
						...
						if(data.offererAccepted == 0) {
							html += '<span class="checkbox"><input type="checkbox" id="creatorCheck" data-id="'+data.todo_id+'" data-offacceptedid="'+data.offerAcceptedId+'"><label for="creatorCheck"></label></span>';
						}else {
							html += '<span> - '+data.tododate1+'</span><span class="checkbox"><input type="checkbox" checked id="creatorCheck" data-id="'+data.todo_id+'" data-offacceptedid="'+data.offerAcceptedId+'"><label for="creatorCheck"></label></span>';
						}
					html += '</p>';
				...
				setTimeout(function() {
					var jroll = new JRoll("#main-content", {
				      scrollBarY: false
				    });
				}, 400);
			}
	...
	$("body").on("click", "#creatorCheck", function() {
		var status = $(this).prop('checked');
		var todoId = $(this).attr('data-id');
		var offerAcceptedId = $(this).attr('data-offacceptedid');

		$("#creatorCompleteModal, #creatorRemoveCompleteModal").attr('data-id', todoId).attr('data-offacceptedid', offerAcceptedId);
		if(status == true) {
	...
	$("#creatorCompleteModal form").submit(function() {
		...
		var acceptedOfferId = $("#creatorCompleteModal").attr('data-offacceptedid');
		...
		$.ajax({
			...
			dataType: "json",
			crossDomain:true, 
			data: {'userid':id, 'todoId': todoId, 'acceptedOfferId': acceptedOfferId},
			success : function(data) {
